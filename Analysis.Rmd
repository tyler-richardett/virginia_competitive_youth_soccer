...
To attribute an "average" location to each youth soccer club, I first had to match a set of coordinates to each field on which at least one game was played. I used functions from the `dplyr` package to combine all address variables into one, and the `street2coordinates` function from the `RDSTK` to translate those addresses into sets of coordinates.
